...
import { weatherBgMap } from '@/utils/weatherUtils'
...
export default function App() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center"
        style={{ background: weatherBgMap.partly_cloudy }}>
  ...
  if (error || !data) {
    return (
      <div className="min-h-screen flex items-center justify-center"
        style={{ background: weatherBgMap.cloudy }}>
        <div className="glass p-6 flex flex-col items-center gap-3">
          <p className="text-sm glass-text-secondary">{error || '暂无数据'}</p>
          <button
            onClick={handleRefresh}
            className="glass-sm glass-hover px-4 py-2 text-sm glass-text"
          >
            重试
          </button>
        </div>
  ...
  const bg = weatherBgMap[data.current.weatherType] || weatherBgMap.partly_cloudy
      ...
      <div className="relative z-10 min-h-screen">
        <div className="max-w-lg mx-auto px-4 py-6 space-y-4 pb-8">
          <header className="flex items-center justify-between">
            ...
            <button
              onClick={handleRefresh}
              className="glass-sm glass-hover p-2 ml-2"
              title="刷新定位"
            >
